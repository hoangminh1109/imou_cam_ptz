...
class ImouEntity(CoordinatorEntity):
    ...
    def __init__(self, coordinator, config_entry, sensor_instance, entity_format) -> None:
        """Initialize."""
        super().__init__(coordinator)
        self.config_entry = config_entry
        self.channel = coordinator.channel
        self.sensor_instance = sensor_instance
        self.entity_id = async_generate_entity_id(
            entity_format,
            f"{self.channel.get_name()}_{self.sensor_instance.get_name()}",
    ...
    def available(self) -> bool:
        ...
        return self.coordinator.channel.get_status()

    @property
    def name(self):
        """Return the name of the sensor."""
        return f"{self.channel.get_name()} {self.sensor_instance.get_description()}"

    @property
    def icon(self):
        """Return the icon of this sensor."""
        if self.sensor_instance.get_type() in SENSOR_ICONS:
            return SENSOR_ICONS[self.sensor_instance.get_type()]
